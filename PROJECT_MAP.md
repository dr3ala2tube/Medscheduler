...
Last updated: 2026-06-11 — M1–M4 complete and live (web merged to `main`, deployed on Render `medscheduler-5io6.onrender.com`; desktop updated with workspace support). **FEATURE CYCLE 2 COMPLETE AND LIVE (M5–M10, deployed 2026-06-10):** legacy-import prompt removed, email verification for new signups, invitation accept/decline/leave with per-user notifications panel, UI alignment fixes. Rules v2 (invites + notifications) published in Firebase Console. **FEATURE CYCLE 3 COMPLETE AND LIVE (M11–M15, deployed 2026-06-11):** conflict detection (duplicate duty/clinic slots, warn + override, red highlights + ⚠ chip), immutable audit trail (`workspaces/{wsId}/audit/{entryId}`) with 📜 History modal, header-row compaction, password reset on sign-in card. Rules v3 (audit block) published in Firebase Console. **FEATURE CYCLE 4 COMPLETE AND LIVE (M16–M20, deployed 2026-06-11):** audit-retention mechanism (`expire_at` + rules v4 published; TTL activation deferred by user), conflict scope v2 (leave-day + missing post-call rest), dark & light themes, Summary tab rework. Live-matrix fixes: `.btn-cancel` dark-mode text color, Summary nested-`<tr>` column alignment.
...
### Feature cycle 4 (branch: `feature/retention-conflicts-themes`, T4: merge approval required) — PLANNED 2026-06-11, AWAITING APPROVAL

Scope (user request 2026-06-11): (1) 90-day audit-log retention, (2) conflict scope extended to leave-day assignments and missing post-call rest, (3) dark & light themes.

Pre-flight: local git index is corrupted by the mount (every tracked file shows staged-deleted + untracked; `index.lock` cannot be unlinked from the sandbox). File contents and `git log` are intact (HEAD 2757963 on `feature/conflicts-audit`). User must repair locally BEFORE branching: delete `.git/index.lock`, run `git reset` (rebuilds index from HEAD), confirm `git status` is clean apart from genuinely new files, and confirm whether cycle 3 was merged to `main` (PROJECT_MAP says yes via GitHub PR; local clone still sits on the feature branch).

- **M16 — Audit retention backend + rules (90 days, Firestore TTL) — IMPLEMENTED 2026-06-11** (L2: additive field + rules republish)
  - app.py: `AUDIT_RETENTION_DAYS = 90`; `_audit()` adds `expire_at` = created + 90 days as a Firestore `timestampValue`; `_py_to_fs` gains `datetime → timestampValue` mapping (additive; no other caller passes datetime)
  - firestore.rules: audit `create` → `hasOnly(['actor_email','created','summary','changes','expire_at'])` + `expire_at is timestamp` **when present** (presence NOT required → old code keeps writing valid entries during the deploy window; deploy order safe both ways, rules-first stays the standard). NO update/delete added — D13 user-facing immutability fully preserved; deletion is Firestore-server-side TTL only
  - User-side (one-time, like rules publishing): enable TTL policy on collection group `audit`, field `expire_at` (Firebase Console → Firestore → TTL, or `gcloud firestore fields ttls update`); document in web/DEPLOY.md
  - Pass (V2): mocked-Firestore — entry carries timestampValue exactly created+90d; diff/no-change/failure paths unchanged; py_compile; rules dry-run in M19
  - VERIFIED 2026-06-11: 17/17 (`test_m16_retention.py`, session outputs, not committed — prior-cycle pattern): `_py_to_fs` datetime→timestampValue Z-normalized (UTC + non-UTC offsets); `_audit` payload = exactly {actor_email, created, summary, changes, expire_at} with expire_at == created+90d; failure path still returns False; wire-level body via mocked urlopen shows `timestampValue` (created stays stringValue — display format unchanged for the History modal); diff engine + no-change-save regression intact; 20 routes. py_compile OK. NOTE: sandbox mount shows stale/truncated views of edited files (known issue, memory), so tests ran on a byte-equivalent patched copy (identical unique-match replacements, diff-verified modulo CRLF); Windows-side files verified complete via direct reads (tails + all edited regions). DEPLOY.md gained the TTL-policy setup section (Console + gcloud) and a pointer that current rules live in firestore.rules (the inline snippet there was the pre-workspace text)
- **M17 — Conflict scope v2: leave-day + post-call rest — IMPLEMENTED 2026-06-11** (L1: index.html only; no API/schema/rules change; warn + override per D11)
  - `computeConflicts()` entries gain `type`: `'slot'` (existing), `'leave'`, `'rest'`; `cells` set unchanged in shape
  - `'leave'`: any WORKING code on a day inside one of that physician's `S.leaves` ranges (f ≤ day_str ≤ t, lexicographic ISO compare like scheduler Phase 1). Working code = anything not in {empty, 'O', 'PC', 'L', 'R'} (mirrors scheduler `OFF_SET`)
  - `'rest'`: duty code (`S.C.duty_set`) on day d → flag any WORKING code on days d+1 … d+`S.rules.post_call_days` (user decision 2026-06-11: working-code-in-window; empty/O/PC/L/R count as rest — matches the scheduler's own `assign_duty` constraint). Both the duty cell and the violating cell(s) highlight
  - Month-boundary limitation (accepted, documented): windows truncate at month edges — a duty in the last `post_call_days` of a month cannot be checked against the next month (cross-month data not loaded); same for leaves spanning months (only in-month days checked)
  - Edit-time warnings: `conflictCreatedBy()`-style checks for all three types in cell-edit Apply + Manual Assign (existing warn-once-then-apply mechanics reused); conflicts modal groups entries by type; ⚠ chip counts all types
  - Pass (V2): node --check; extracted-function logic tests (leave overlap in/out, working vs O/PC/empty in window, post_call_days=0/1/3, duty at month end truncation, combined slot+leave on same cell single highlight); render test
  - VERIFIED 2026-06-11: node --check OK on full inline JS; 29/29 logic tests (`test_m17_conflicts.js`, session outputs — extracts the conflict block verbatim): all M12 slot regressions (2×DM, DM+DF ok, own-cell reassign, cells set) + leave (boundaries inclusive, L/O/PC exempt, other-physician leave irrelevant, string-pid coercion, cross-month range lexicographic) + rest (PC/O/empty = rest, multiple in-window violations, day-after-window exempt, consecutive duties flagged, month-end truncation, pc=0/1, duty+violating cells both highlighted) + cellConflictWarnings in all four directions (slot dup, on-leave, placing work inside an existing rest window, placing a duty whose window is blocked) + typed ordering slot<leave<rest per day; Flask render 200 with engine v2, all prior ids, typed modal texts, 401s intact. Implementation notes: single physician-loop computes all three types in one O(cells) pass (leaves checked per working cell, small array); warn texts joined with ' · ' in the existing .modal-err areas, warn-once mechanics (S.editing.conflictWarned / manualWarnKey) unchanged; rest entries carry dutyDay/dutyCode for the modal text; slot modal line now prefixed "Duplicate" (display-only change)
- **M18 — Dark & light themes — IMPLEMENTED 2026-06-11** (L1: index.html only)
  - `:root` = light (unchanged values); `html[data-theme="dark"]` overrides the existing variables (`--bg --surface --border --text --muted --primary --primary-dark --danger --success --warn`) plus new vars extracted from currently hardcoded colors (auth gradient/card, header overlays, modal/toast surfaces, table zebra/hover) — sweep of the ~115 `var(--…)` usages + hardcoded hex/rgba in CSS
  - Schedule cells keep the user's per-code `color_map` pastel backgrounds in BOTH themes (they are data, not chrome) with forced dark cell text for contrast; `.cell-conflict` outline still `var(--danger)`
  - 🌓 three-state toggle (auto → light → dark) in header row 1 (icon-only, next to 🔔/📜 pattern; row already scrolls at ≤640px per M9); preference in `localStorage('ms-theme')`, default `auto` = `prefers-color-scheme`; `<meta name="theme-color">` updated on switch; auth screen included (toggle reachable pre-login on the auth card)
  - Pass (V2): node --check; render test (new ids); manual spot-check both themes at 360/640/900/desktop in M19 matrix
  - VERIFIED 2026-06-11: node --check OK (both script blocks incl. the new head FOUC guard); 11/11 theme-logic tests (`test_m18_theme.js`, session outputs — extracts the theme block verbatim with stubbed document/localStorage/matchMedia): default auto, auto follows system both ways, full cycle auto→light→dark→auto with persistence + icon/title updates, explicit pref ignores system, system-change listener re-applies only in auto, garbage localStorage value falls back to auto, theme-color meta swaps #2563eb↔#0f172a; render 200 with dark palette block, all tint vars in :root, both toggle ids, FOUC guard inside <head>, conflict outline + M17 engine intact, 401s intact. Implementation notes: 8 new CSS vars (--hl/--hl-border/--danger-soft/--thead/--auth-g1/--auth-g2/--toast-bg/--toast-fg) so light theme is byte-identical in rendering; dark overrides chrome vars only (--primary/--danger/--success/--warn unchanged for brand + conflict-outline contrast); schedule grid stays light in both themes (day-cell + grid th text pinned #1a202c; weekend tints untouched) per D16 — summary table and toast ARE themed (no color-coded data there); dark-scoped element rule themes form controls (header #ws-select wins by ID specificity); 🌓 button in header user-badge + .theme-fab on the auth overlay, both driven by the same cycleTheme; sweep checked: only remaining hardcoded tints are var definitions + the intentional weekend #fffbeb
- **M19 — V3 verification + deploy + docs — DONE 2026-06-11 (V3: live operational verification, user-confirmed)**
  - LIVE 2026-06-11 (user-confirmed: "everything achieved"): rules v4 published before merge; branch merged via PR → Render deploy; TTL deliberately NOT enabled (D14 amendment); live matrix passed — conflicts v2 (leave-day + post-call) warn/override/highlight, themes persist and follow OS in Auto, Summary tab aligned with new M20 columns, both findings fixed during the matrix (btn-cancel color, nested-tr alignment)
  - Pre-deploy regression: re-run cycle-3 suites (28 audit backend + conflict logic extended + relTime), py_compile, render checks, route count, working-tree blobs == HEAD (truncation check per memory)
  - Rules Playground dry-run: audit create WITH expire_at (member, own email) OK; WITHOUT expire_at OK (transition window); forged actor_email rejected; update/delete still rejected; non-member read rejected
  - Deploy order: publish rules → merge PR → Render deploy → ~~user enables TTL policy~~ **TTL activation deferred (D14 amendment)** → live matrix: save produces entry with `expire_at` visible in Console (verifies the mechanism even without a policy); old entries untouched; leave-day + post-call conflicts warn/highlight/override on both accounts; theme toggle persists across reload and both accounts/devices; both themes at 360/640/900/desktop
  - Update PROJECT_MAP.md + PROJECT_STATE.md after live matrix passes (cycle-2/3 pattern)
  - M19 live-matrix finding #2 (user, 2026-06-11): Summary tab columns misaligned — numbers not under their headers. PRE-EXISTING structural bug exposed by dark-mode borders: `#summary-head` is the `<tr>` inside `<thead>`, but `renderSummary()` appended a SECOND `<tr>` into it; the browser wraps the nested row in an anonymous table inside column 1, so the header rendered as its own mini-table and the body's first column inflated to its width. Fix: append the `<th>`s directly to the existing row (nested-tr creation removed). Verified: 6/6 structural tests (`test_m19_summary.js`, extracts renderSummary verbatim; header = 9 direct th, no nested tr, values under correct columns, `.high` follows max_hours), node --check, render 200
  - M19 live-matrix finding (user, 2026-06-11): secondary modal buttons (Cancel / Close / Reset to Defaults, `.btn-cancel`) unreadable in dark mode — the rule set `background:var(--bg)` but no `color`, so the UA-default black applied (buttons don't inherit text color). Fix: `color:var(--text)` added to `.btn-cancel`; audited all other button rules for the same gap (none — every other button base rule sets a color). Light theme unchanged (`--text` = the previous effective black)
  - PRE-DEPLOY REGRESSION 2026-06-11 (sandbox, on copies byte-equivalent to the working tree): 17/17 M16 retention + 29/29 M17 conflicts + 11/11 M18 theme; py_compile (app, scheduler); node --check (both script blocks); render 200 with ALL cycle-3 ids (conflict/history/notif/ws/verify/forgot) + cycle-4 ids (btn-theme, btn-theme-auth) + all JS hooks; 20 routes; /api/data, /api/audit, /api/workspaces, /api/notifications all 401 unauthenticated; /api/constants + /api/rules/defaults 200; AUDIT_RETENTION_DAYS=90 wired. Windows-side files verified by direct reads (tails: app.py `app.run(...)` @1125, firestore.rules closing braces, index.html `</html>` @2484). GitHub + Firebase unreachable from sandbox — push/PR/rules publish/TTL/live-matrix are user-side; PROJECT_STATE.md update deferred until live matrix passes

- **M20 — Summary tab columns rework (user-requested 2026-06-11) — IMPLEMENTED 2026-06-11** (L2: scheduler.py compute_summary + index.html renderSummary; same branch)
  - Columns now: Physician, **Morning Days** (replaces Team), Total Hrs, On-Calls, Daycare, Post-Call, **Random Off** (renamed from Off, same O+R data), Leave, **Weekends Inc in Offs** (renamed from Weekend Off, same data), **Total Off Days** (NEW, last)
  - User-confirmed definitions: Morning Days = ALL 8-hr work days (every non-duty code with hours — clinics included, so a clinic day appears in both Morning Days and Daycare); Total Off Days = OFF_SET days = postcall + off(O+R) + leave, consistent with Weekends-Inc-in-Offs being its weekend subset
  - compute_summary: new `morning` day-counter on the existing `elif h>0` branch + `"morning"` field in rows (`team` field kept for compat); total_off computed client-side from existing fields (no extra backend field)
  - VERIFIED 2026-06-11: 11/11 scheduler tests (`test_m20_summary.py` — morning incl. clinics/excl. duties, off/random/leave/postcall/weekend_off counts incl. corrected June-2026 weekend calendar, total_off=6 case, team compat) + 7/7 frontend structural tests (10 direct th, renamed labels, Total Off Days last, no Team column, total_off=14 in last cell); py_compile; node --check; render 200. NOTE: desktop app summary is separate code — unaffected, web-only (consistent with cycle scope)

- **D14 — Audit retention = Firestore native TTL on `expire_at` (created + 90 days), user decision 2026-06-11.** **AMENDED 2026-06-11: TTL policy activation DEFERRED (user decision)** — the mechanism ships fully (every new entry carries `expire_at`; rules v4 accept it) but no TTL policy is enabled, so history is kept indefinitely for now. Rationale: trivial volume at current scale, accountability value beyond 90 days, zero-code future activation. CAVEAT recorded: activating later purges retroactively — all entries older than 90 days at activation time are deleted within ~24 h. Activation steps remain documented in DEPLOY.md. Server-side deletion preserves D13's no-user-deletes immutability; zero purge code; rules keep denying update/delete. Rejected alternative: Flask-side purge with rules permitting old-entry deletes — more code and weakens immutability (a member could delete >90d entries directly). Accepted: TTL deletes lag expiry by up to ~24h (Google-documented typical window); cycle-3 entries lacking `expire_at` never auto-expire — one-time manual Console delete optional → [ORPHANS].
- **D15 — Conflict scope v2 stays client-side, derived, warn + override (extends D11; no persistence, no API change).** "Working code" defined as ∉ {empty, O, PC, L, R} = scheduler `OFF_SET` ∪ empty (user decision 2026-06-11: working-code-in-window for post-call; strict must-be-PC rejected as noisy on manually edited rotas). Leave conflicts derive from `S.leaves` ranges, not from 'L' grid stamps, so an assignment overwriting a stamped L is still caught. Specialty duplicates remain excluded (unchanged user decision).
- **D16 — Theming via CSS custom properties + `html[data-theme]`; preference client-only in `localStorage`, default follows `prefers-color-scheme`.** Not synced to Firestore (per-device preference; no schema/rules change; trivially reversible). Per-code schedule cell colors are data, not theme — identical in both modes with forced dark cell text. Desktop app unaffected.

**Assumptions (cycle 4):**
- [V] `post_call_days` configurable and present in frontend `S.rules` (default 3) — index.html DEFAULT_RULES + rules modal.
- [V] Non-working codes = `OFF_SET` {PC, O, L, R} — scheduler.py line 11; scheduler's own post-call guard skips exactly these plus pinned codes.
- [V] `S.leaves` ranges are `{pid, f, t}` ISO date strings, compared lexicographically — same convention as scheduler Phase 1 leave stamping.
- [A] Firestore TTL policies are available on project `medscheduler-e0853` (standard for Native-mode Firestore); user verifies in Console during M19 before relying on retention.
- [A] 90 days is a fixed constant, not user-configurable (simplest; revisit only if requested).
- [A] No theme preference syncing across devices is expected (localStorage per device).

**Risks (cycle 4):**
- TTL deletion is permanent and unrecoverable once it runs — this is the requested retention behavior, but it is an irreversible-data-deletion class change: enabling the TTL policy is the explicit human-approval gate (user performs it in Console at M19).
- Rules republish required (v4); ordering safe in both directions because `expire_at` is optional in rules and audit writes are best-effort (`audited:false` worst case).
- Dark theme touches many selectors; visual regression risk mitigated by variables-only overrides + both-theme spot-check matrix.
- New conflict types may heavily flag existing/historical months on first load (e.g., rotas where post-call was handled informally) — warn-only, zero data change; users can ignore or override.
- Local git index corruption (pre-flight) must be repaired by the user before any commits, or milestone commits cannot be made safely.

**Rollback (cycle 4):** restore rules v3 from Firebase Console history; disable the TTL policy in Console (stops future deletions; entries already TTL-deleted are gone — intended retention semantics); revert branch/PR (Render redeploys old code); `expire_at` is additive (old code/rules ignore it); conflicts v2 and themes are client-only and leave no data behind.
...
- ~~(Cycle 3) Audit log retention~~ SHIPPED cycle 4 M16: mechanism live (`expire_at` on every new entry, rules v4); TTL policy activation deferred — see the dedicated bullet below.
- ~~(Cycle 3) Conflict scope limited to duplicate slots~~ SHIPPED cycle 4 M17 (leave-day + post-call rest); specialty duplicates remain intentionally excluded (user decision 2026-06-11).
- (Cycle 4) Cycle-3 audit entries written before M16 lack `expire_at` and would never auto-expire even if TTL is activated later — optional one-time manual delete in Firebase Console.
- (Cycle 4) TTL policy activation deferred (D14 amendment, user decision 2026-06-11): audit history currently kept indefinitely; `expire_at` is written on every new entry, so retention can be turned on anytime via the DEPLOY.md steps. WARNING for whoever activates it later: all entries already older than 90 days are purged within ~24 h of activation.
- (Cycle 4) Conflict windows truncate at month boundaries (cross-month data not loaded): a duty in the last `post_call_days` of a month, or a leave spanning months, is only checked against in-month days.
- RESOLVED 2026-06-11: the "git index corruption" (all files staged-deleted + untracked, locked index) existed only in the sandbox's mounted view — no `index.lock` on the real disk; a plain `git reset` user-side rebuilt the index and `git status` shows exactly the 4 expected modified files. Same root cause as the stale/truncated mount reads: do NOT trust sandbox-side git status or file contents for edited files; verify on the Windows side.
