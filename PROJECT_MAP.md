...
- **M19 — V3 verification + deploy + docs**
  ...
  - Deploy order: publish rules → merge PR → Render deploy → user enables TTL policy → live matrix: save produces entry with `expire_at` visible in Console; old entries untouched; leave-day + post-call conflicts warn/highlight/override on both accounts; theme toggle persists across reload and both accounts/devices; both themes at 360/640/900/desktop
