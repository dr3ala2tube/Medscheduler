...
- **M19 — V3 verification + deploy + docs**
...
- (Cycle 3) Audit log has no retention/purge policy (reads capped at 50 newest); entries are immutable and grow unbounded — **scheduled: cycle 4 M16 (90-day TTL)**.
- (Cycle 3) Conflict scope limited to duplicate duty/clinic slots; assignment-during-leave and missing post-call rest **scheduled: cycle 4 M17**; specialty duplicates remain intentionally excluded (user decision 2026-06-11).
- (Cycle 4, on completion) Cycle-3 audit entries written before M16 lack `expire_at` and never auto-expire — optional one-time manual delete in Firebase Console.
...
- (Cycle 4, on completion) Conflict windows truncate at month boundaries (cross-month data not loaded): a duty in the last `post_call_days` of a month, or a leave spanning months, is only checked against in-month days.
