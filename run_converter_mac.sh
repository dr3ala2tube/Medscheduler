#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
#  run_converter_mac.sh  –  Quick-run RotaConverter directly (no build step)
#
#  Usage
#  -----
#    chmod +x run_converter_mac.sh
#    ./run_converter_mac.sh
#
#  Works with Python installed via Homebrew, python.org, or pyenv.
#  Dependencies are installed inside a local .venv_conv/ folder so that
...
VENV_DIR="$SCRIPT_DIR/.venv_conv"

if [ ! -d "$VENV_DIR" ]; then
    info "Creating virtual environment at .venv_conv/ ..."
...
# ── 4. Launch the app ─────────────────────────────────────────────────────────
echo -e "${GREEN}Launching RotaConverter...${NC}"
exec "$VENV_PY" "$SCRIPT_DIR/rota_converter.py"
